...
>Check out the Framer Motion version on codesandbox.io [here](https://codesandbox.io/s/staggered-animation-w-framer-motion-8clmm?file=/src/components/BlogCard.tsx)
...
the `${Pill}` syntax. `${Pill}` is translated into the generated class name for Pill by @emotion/styled. Referencing one styled component from another can be found in the official docs
