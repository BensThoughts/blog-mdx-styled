...
styled-component component to show how this can be done using ::before in css. As a bonus I have also
shown one method that can be used to make sure that the background of our container element lightens up
on both keyboard activation of an inner element and hover using react useState and useEffect hooks.
...
make sure that it doesn't cover up all of the other elements and text in the container. By tracking an
activated state we make our component more accessible. Both focus or an inner element and hover will now
lighten the container. The final results for my particular use of this container can be seen below.
