...
Photo by <a href="https://unsplash.com/@ventiviews?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText">Venti Views</a> on <a href="https://unsplash.com/s/photos/container?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText">Unsplash</a>
...
correctly. That is **A LOT** of users that are going to get a horrible experience if you try to use
container queries without a fallback.

So clearly a fallback is needed. I looked into using a polyfill such as
[container-query-polyfill](https://github.com/GoogleChromeLabs/container-query-polyfill) but it was
becoming to complicated to setup correctly, had too many caveats, and would add more JS \(bloat\). Also
I don't need my sites to be perfectly identical for all users. I'm ok with showing a very good layout to
some users and a pitch perfect amazing layout to others. I just need my fallback to work for **most**
users. I would also like to be able to use tailwindcss because tailwind is highly scalable, easy to
update, and I already use it across multiple frameworks and sites that I run.
...
most users. The solution is going to be to implement container queries and use media queries as a fallback.
...
your media query with `@container`. You can then conditionally style any children elements of that
...
But wait, not so fas, this doesn't work! The problem is that the media queries take precedent over the
...
work I did to make my elements flow in a row based on the container size is undone.
...
The biggest issue I ran into while developing was that my code would not take effect until I re-compiled
(ran the build step) on my code. This was especially evident with with [remix](https://remix.run/). I
thought it was not working because the changing anything about the `supports-[]` or `@[600px]:` classes
would causes styling to be lost. For example if I
had to change `@[600px]:flex-row` to `@[500px]:flex-row` the live reload wouldn't show any change and in
fact would loose all of the container breakpoint styling. The App needed to be re-built to trigger
re-building the tailwindcss files.

The caveats are of course that you may still end up with some funky stuff in the edge cases with users
whose browser doesn't support container queries. There is also the fact that simply testing if the
browser supports `container-type: inline-size` may not be a completely fool proof solution to determine
if a browser fully supports container queries correctly.
